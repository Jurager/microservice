...
Laravel package for building service meshes over HTTP and RabbitMQ. It provides ECDSA-signed inter-service HTTP communication, backed by a cluster CA, with manifest-driven gateway routing, plus an event bus for asynchronous fan-out between services. The package is opt-in feature-by-feature — services that don't need the bus, the gateway, retries, or even idempotency pay nothing for those subsystems.

You may use the package to stitch together a small handful of services or a large cluster — the conventions stay the same either way, regardless of how the cluster's network is segmented. Each service signs HTTP requests and event envelopes with its own key pair and carries a certificate, issued by the cluster's CA, proving that key belongs to it. The same `MessageHandler` contract drives every consumer.
